...
exports.TTSQueue = void 0;
const events_1 = require("events");
const tts_streaming_1 = require("./tts-streaming");
// Emit a warning if the queue exceeds this depth
const QUEUE_WARNING_THRESHOLD = 5;
class TTSQueue extends events_1.EventEmitter {
    constructor(config) {
        super();
        this.queue = [];
    ...
    async processNext() {
        ...
        console.log(`[TTSQueue] Processing sentence ${item.sentenceIndex}: ` +
        ...
        try {
            ...
            ttsService.on("error", (error) => {
                console.error(`[TTSQueue] TTS error on sentence ${item.sentenceIndex}:`, error.message);
                if (!this.isCancelled) {
            ...
            await new Promise((resolve, reject) => {
                ...
                setTimeout(() => {
                    reject(new Error(`TTS timed out on sentence ${item.sentenceIndex}`));
                }, 30000);
            });
            // Emit sentence completion with latency
            const latencyMs = Date.now() - startTime;
            console.log(`[TTSQueue] Sentence ${item.sentenceIndex} done in ${latencyMs}ms`);
            ...
            console.error(`[TTSQueue] Failed on sentence ${item.sentenceIndex}:`, errorMessage);
            if (!this.isCancelled) {
...
exports.TTSQueue = TTSQueue;
//# sourceMappingURL=tts-queue.js.map