...
const OUTPUT_FILE = path.join(__dirname, "../../test-pipeline-output.mp3");
 ...
 */
async function streamTokens(ws) {
    ...
    for (let i = 0; i < LLM_TOKENS.length; i++) {
        ...
        console.log(`[Tokens] ${i + 1}/${LLM_TOKENS.length}: "${token}"` +
            `${isLast ? " (+ flush)" : ""}`);
...
async function testPipeline() {
    ...
    console.log(`Expected: 2 sentences split by chunker`);
            ...
            switch (message.type) {
                ...
                case "audio_chunk": {
                    ...
                    if (audioChunks.length % 5 === 0) {
                        console.log(`[Audio] Received ${audioChunks.length} chunks ` +
                ...
                case "sentence_end": {
                    ...
                    console.log(`[Sentence ${message.data.sentenceIndex}] ` +
                ...
                case "stream_end": {
                    ...
                    console.log("========================================");
                    console.log(`Sentences detected:    ${sentencesStarted}`);
                    console.log(`Sentences completed:   ${sentencesCompleted}`);
                    console.log(`Total audio chunks:    ${audioChunks.length}`);
                    console.log(`Audio file size:       ${(fullAudio.length / 1024).toFixed(1)}KB`);
                    console.log(`First chunk latency:   ${firstChunkMs}ms`);
                    ...
                    for (const sl of sentenceLatencies) {
                        console.log(`  Sentence ${sl.index}: ${sl.latencyMs}ms — ` +
                            `"${sl.text.substring(0, 50)}"`);
                    }
                    console.log(`\nAudio saved: ${OUTPUT_FILE}`);
                    // Determine pass/fail
                    const passed = sentencesStarted >= 2 &&
                    ...
                    if (passed) {
                        ...
                        if (sentencesStarted < 2) {
                            console.log("  → Expected 2+ sentences, got " +
                                sentencesStarted);
                        }
