 ...
 */
function runSession(sessionId, text) {
...
async function main() {
    ...
    console.log(`Launching ${CONCURRENT_SESSIONS} sessions simultaneously...\n`);
    const startTime = Date.now();
    const promises = Array.from({ length: CONCURRENT_SESSIONS }, (_, i) => runSession(i, TEST_TEXTS[i]));
    ...
    console.log("Session  Status   Chunks  Audio     1st Chunk  Total");
    console.log("─".repeat(70));
    for (const r of results) {
        const status = r.success ? "PASS" : "FAIL";
        const audioKB = (r.audioBytes / 1024).toFixed(1);
        console.log(`  ${String(r.sessionId).padStart(2)}     ${status.padEnd(6)}  ` +
    ...
    const avgFirstChunk = passed.length > 0
        ? Math.round(passed.reduce((s, r) => s + r.firstChunkMs, 0) /
            passed.length)
        : 0;
    const avgTotal = passed.length > 0
        ? Math.round(passed.reduce((s, r) => s + r.totalMs, 0) /
            passed.length)
        : 0;
    const totalChunks = results.reduce((s, r) => s + r.audioChunks, 0);
    const totalAudioKB = (results.reduce((s, r) => s + r.audioBytes, 0) / 1024).toFixed(1);
    console.log("\n==========================================");
