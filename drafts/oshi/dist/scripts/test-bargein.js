...
async function testBargeIn() {
    ...
    return new Promise((resolve) => {
            ...
            switch (msg.type) {
                ...
                case "audio_chunk": {
                    if (!interrupted) {
                        ...
                        console.log(`[2] Audio chunk ${chunksBeforeInterrupt} received`);
                        ...
                        console.warn(`[WARN] Audio chunk received AFTER interrupt!`);
                    }
                    break;
                }
                case "stop_audio": {
                    ...
                    console.log(`[4] stop_audio received in ${stopAudioLatencyMs}ms ` +
                        `(reason: ${msg.data.reason})`);
                    break;
                }
                case "cancelled": {
                    cancelledReceived = true;
                    console.log(`[4] cancelled received — requestId: ${msg.data.requestId}`);
                ...
                case "sentence_start": {
                    console.log(`[Sentence ${msg.data.sentenceIndex}] TTS started: ` +
                        `"${msg.data.text.substring(0, 40)}..."`);
                    break;
                }
                case "sentence_end": {
                    console.log(`[Sentence ${msg.data.sentenceIndex}] done in ` +
        ...
        function printResults() {
            ...
            console.log(`Chunks after interrupt:  ${chunksAfterInterrupt}`);
            console.log(`stop_audio received:     ${stopAudioReceived ? "yes" : "NO"}`);
            console.log(`cancelled received:      ${cancelledReceived ? "yes" : "NO"}`);
            console.log(`Cancel latency:          ${stopAudioLatencyMs}ms`);
            const passed = stopAudioReceived &&
                cancelledReceived &&
                chunksAfterInterrupt === 0;
            if (passed) {
                console.log("\n✅ PASS — Barge-in works correctly");
                console.log("   Audio stopped immediately, no chunks leaked");
            }
            else {
                console.log("\n❌ FAIL");
                ...
                if (chunksAfterInterrupt > 0) {
                    console.log(`   → ${chunksAfterInterrupt} chunks leaked after interrupt`);
                }
            }
            resolve(passed);
        }
...
//# sourceMappingURL=test-bargein.js.map