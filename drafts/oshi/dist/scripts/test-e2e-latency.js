        ...
        ws.on("message", (raw) => {
            ...
            switch (msg.type) {
                case "sentence_start": {
                    ...
                    console.log(`[Sentence ${msg.data.sentenceIndex}] ` +
                ...
                case "stream_end": {
                    ...
                    const result = {
                        firstTokenToFirstChunkMs: firstChunkTime - firstTokenTime,
                        firstTokenToStreamEndMs: streamEndTime - firstTokenTime,
                        sentenceBoundaryDetectedMs: firstSentenceTime - firstTokenTime,
                        perSentence,
                        totalChunks,
                        totalAudioKB: Math.round((totalAudioBytes / 1024) * 10) / 10,
                    };
                    printResults(result);
                    ws.close();
                    resolve(result);
...
function printResults(result) {
    ...
    console.log("==========================================");
    console.log("\nPipeline timing:");
    console.log(`  First token → sentence detected:  ` +
        `${result.sentenceBoundaryDetectedMs}ms`);
    console.log(`  First token → first audio chunk:  ` +
        `${result.firstTokenToFirstChunkMs}ms`);
    console.log(`  First token → all audio done:     ` +
        `${result.firstTokenToStreamEndMs}ms`);
    console.log("\nPer-sentence TTS latency:");
    for (const s of result.perSentence) {
        console.log(`  Sentence ${s.index}: ${s.ttsLatencyMs}ms — "${s.text}"`);
    }
    console.log(`\nTotal chunks: ${result.totalChunks}`);
    console.log(`Total audio:  ${result.totalAudioKB}KB`);
    // Grade against targets
    console.log("\n── Target Check ──");
    const firstChunkTarget = 1200;
    const firstChunkStatus = result.firstTokenToFirstChunkMs <= firstChunkTarget
        ? "PASS" : "SLOW";
    console.log(`  First audio chunk < ${firstChunkTarget}ms: ` +
        `${result.firstTokenToFirstChunkMs}ms [${firstChunkStatus}]`);
    const totalTarget = 3000;
    const totalStatus = result.firstTokenToStreamEndMs <= totalTarget
        ? "PASS" : "SLOW";
    console.log(`  Total pipeline < ${totalTarget}ms:     ` +
        `${result.firstTokenToStreamEndMs}ms [${totalStatus}]`);
    if (firstChunkStatus === "PASS" &&
        totalStatus === "PASS") {
...
async function main() {
    ...
    // Average
    const avgFirstChunk = Math.round(results.reduce((s, r) => s + r.firstTokenToFirstChunkMs, 0) /
        results.length);
    const avgTotal = Math.round(results.reduce((s, r) => s + r.firstTokenToStreamEndMs, 0) /
        results.length);
    console.log("\n==========================================");
...
//# sourceMappingURL=test-e2e-latency.js.map