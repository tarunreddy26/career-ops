...
export const DEFAULT_GENKI_VALUE = 80;
...
export function applyDailyCheckin(
  ...
  const bonusApplied = Math.min(
    DAILY_CHECKIN_BONUS,
    GENKI_CAP - currentGenki
  );
  const newGenki = Math.min(
    currentGenki + DAILY_CHECKIN_BONUS,
    GENKI_CAP
  );
 ...
 */
export function applyMessageBonus(
  currentGenki: number,
  sessionMessagePoints: number
): MessageRestoreResult {
  ...
  const remainingSessionCap =
    MESSAGE_BONUS_CAP - sessionMessagePoints;
  const pointsToAdd = Math.min(MESSAGE_BONUS, remainingSessionCap);

  // Cap at 100 — genki never exceeds max
  const actualPointsAdded = Math.min(
    pointsToAdd,
    GENKI_CAP - currentGenki
  );
 ...
 */
export function getGenkiTier(genkiValue: number): GenkiTier {
  if (genkiValue >= 76) return "full_energy";
  if (genkiValue >= 51) return "good";
  if (genkiValue >= 26) return "tired";
  return "very_tired";
}
 ...
 * | 7+        | long_absence    |
 */
export function getReturnDialogueTier(
  daysElapsed: number
): ReturnDialogueTier {
  if (daysElapsed <= 0) return "no_absence";
  if (daysElapsed <= 2) return "short_absence";
  if (daysElapsed <= 6) return "medium_absence";
  return "long_absence";
}
 ...
 * @param now - Current time (injectable for testing)
 */
export function processSessionOpen(
  genkiValue: number,
  lastActiveAt: Date,
  lastCheckinAt: Date,
  now: Date = new Date()
  ...
  const restoreResult = applyDailyCheckin(
    decayResult.new_genki_value,
    lastCheckinAt,
    now
  );
  ...
  const returnDialogueTier = getReturnDialogueTier(
    decayResult.days_elapsed
  );
