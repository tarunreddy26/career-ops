...
import { DEFAULT_GENKI_VALUE } from "./genki-meter";
...
interface UserRow {
  ...
  user_id: string;
  genki_value: number;
  last_active_at: string;  // ISO timestamp from Postgres
  last_checkin_at: string;  // ISO timestamp from Postgres
  created_at: string;
  updated_at: string;
}
 ...
 */
export async function getUserGenkiState(
  userId: string
): Promise<GenkiUserState> {
...
async function createNewUser(
  ...
  const newState: GenkiUserState = {
    genki_value: DEFAULT_GENKI_VALUE,
    last_active_at: now,
  ...
  console.log(`[Supabase] New user created: ${userId} (genki: ${DEFAULT_GENKI_VALUE})`);
  return newState;
}
 ...
 */
export async function updateUserGenkiState(
  userId: string,
  updates: {
    genki_value?: number;
    last_active_at?: Date;
