...
async function runTTSTest(): Promise<void> {
  ...
  ttsService.on("streamStart", () => {
    ...
    console.log(
      `[TEST] First audio chunk received — ${latency}ms`
    );
  });
  ...
  try {
    ...
    console.log(
      `First chunk latency: ${firstChunkLatency}ms`
    );
    ...
    if (
      typeof firstChunkLatency === "number" &&
      firstChunkLatency < 200
    ) {
      console.log(
        "PASS — First chunk under 200ms target!"
      );
    } else if (
      typeof firstChunkLatency === "number" &&
      ...
      console.log(
        "WARN — First chunk under 500ms but above 200ms target"
      );
    } else {
      console.log(
        "SLOW — First chunk above 500ms, investigate latency"
      );
    }
  } catch (error) {
    const message =
