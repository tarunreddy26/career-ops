...
const TEST_TEXT =
  "Hey! Welcome to the stream, glad you're here! " +
 ...
 */
async function testTTSStreaming(): Promise<boolean> {
    ...
    ws.on("message", (rawData) => {
      ...
      switch (message.type) {
        case "stream_start": {
          ...
          console.log(
            `[OK] stream_start — requestId: ${requestId}`
          );
          break;
        }
        ...
        case "audio_chunk": {
          ...
          if (audioChunks.length === 1) {
            ...
            console.log(
              `[OK] First audio chunk in ${firstChunkTime}ms ` +
        ...
        case "stream_end": {
          ...
          console.log("\n=== Test 1 Results ===");
          console.log(
            `stream_start received: ${streamStartReceived}`
          );
          console.log(
            `First chunk latency: ${firstChunkTime}ms`
          );
          ...
          console.log(
            `Audio size: ${(fullAudio.length / 1024).toFixed(1)}KB`
          );
        ...
        case "error": {
          console.error(
            `[FAIL] Error: ${message.data.message} ` +
 ...
 */
async function testCancel(): Promise<boolean> {
  console.log("\n=== Test 2: Cancel Flow ===");
  ...
  return new Promise<boolean>((resolve) => {
    ...
    ws.on("message", (rawData) => {
      ...
      if (message.type === "stream_end") {
        console.log(
          "[OK] Stream finished before cancel " +
    ...
    ws.on("close", () => {
      ...
      if (!cancelConfirmed) {
        console.log(
          "[OK] Connection closed (cancel may have " +
