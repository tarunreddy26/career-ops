...
(() => {
  const result = calculateDecay(80, hoursAgo(23, NOW), NOW);
...
// 1 day decay: -10
(() => {
  const result = calculateDecay(80, daysAgo(1, NOW), NOW);
...
// 2 day decay: -20
(() => {
  const result = calculateDecay(80, daysAgo(2, NOW), NOW);
...
// 5 day decay: -50
(() => {
  const result = calculateDecay(80, daysAgo(5, NOW), NOW);
...
(() => {
  const result = calculateDecay(80, daysAgo(7, NOW), NOW);
  assert(
    "7-day max decay: 80 → 10 (ember)",
    result.new_genki_value,
    10
  );
...
(() => {
  const result = calculateDecay(80, daysAgo(14, NOW), NOW);
  assert(
    "14-day decay still capped at 70: 80 → 10",
    result.new_genki_value,
    10
  );
...
(() => {
  const result = calculateDecay(15, daysAgo(3, NOW), NOW);
  assert(
    "Decay floors at 0: 15 - 30 → 0",
    result.new_genki_value,
    0
  );
...
(() => {
  const result = calculateDecay(0, daysAgo(5, NOW), NOW);
...
(() => {
  ...
  const result = applyDailyCheckin(60, yesterday, NOW);
  assert(
    "Daily check-in: 60 → 80",
    result.new_genki_value,
    80
  );
...
(() => {
  ...
  const result = applyDailyCheckin(60, earlierToday, NOW);
  assert(
    "Same day: no bonus, stays 60",
    result.new_genki_value,
    60
  );
...
(() => {
  const yesterday = new Date("2026-03-24T18:00:00Z");
  const result = applyDailyCheckin(90, yesterday, NOW);
  assert(
    "Cap at 100: 90 + 20 → 100",
    result.new_genki_value,
    100
  );
...
(() => {
  const yesterday = new Date("2026-03-24T18:00:00Z");
  const result = applyDailyCheckin(100, yesterday, NOW);
  assert(
...
// First message: +2
(() => {
  const result = applyMessageBonus(60, 0);
...
(() => {
  const result = applyMessageBonus(68, 8);
...
(() => {
  const result = applyMessageBonus(70, 10);
  assert(
    "6th msg: capped, stays 70",
    result.new_genki_value,
    70
  );
...
(() => {
  const result = applyMessageBonus(99, 0);
  assert(
    "Message at 99: caps at 100",
    result.new_genki_value,
    100
  );
...
assert("100 → full_energy", getGenkiTier(100), "full_energy");
assert("76 → full_energy", getGenkiTier(76), "full_energy");
assert("75 → good", getGenkiTier(75), "good");
assert("51 → good", getGenkiTier(51), "good");
assert("50 → tired", getGenkiTier(50), "tired");
assert("26 → tired", getGenkiTier(26), "tired");
assert("25 → very_tired", getGenkiTier(25), "very_tired");
assert("0 → very_tired", getGenkiTier(0), "very_tired");
...
assert("0 days → no_absence",
  getReturnDialogueTier(0), "no_absence");
assert("1 day → short_absence",
  getReturnDialogueTier(1), "short_absence");
assert("2 days → short_absence",
  getReturnDialogueTier(2), "short_absence");
assert("3 days → medium_absence",
  getReturnDialogueTier(3), "medium_absence");
assert("5 days → medium_absence",
  getReturnDialogueTier(5), "medium_absence");
assert("6 days → medium_absence",
  getReturnDialogueTier(6), "medium_absence");
assert("7 days → long_absence",
  getReturnDialogueTier(7), "long_absence");
assert("30 days → long_absence",
  getReturnDialogueTier(30), "long_absence");
...
(() => {
  ...
  assert("Session open: decay + restore = 90",
    result.genki_value, 90);
  assert("Tier: full_energy", result.genki_tier, "full_energy");
  assert("1 day elapsed", result.days_elapsed, 1);
  assert("Short absence dialogue",
    result.return_dialogue_tier, "short_absence");
})();
...
(() => {
  const result = processSessionOpen(
    80,
    daysAgo(7, NOW),       // last active 7 days ago
    daysAgo(7, NOW),       // last checkin 7 days ago
    NOW
  );
  // Decay: 7 days × 10 = 70 (capped at 70) → 80 - 70 = 10
  // Daily bonus: +20 → 10 + 20 = 30
  assert("7-day absence: 80 → 10 → 30",
    result.genki_value, 30);
  assert("Tier: tired", result.genki_tier, "tired");
  assert("Long absence dialogue",
    result.return_dialogue_tier, "long_absence");
})();
...
(() => {
  ...
  const result = processSessionOpen(
    75,
    earlierToday,    // active earlier today
    earlierToday,    // checked in earlier today
    NOW
  );
  // No decay (< 24h), no daily bonus (same day)
  assert("Same-day return: stays 75",
    result.genki_value, 75);
  assert("Tier: good", result.genki_tier, "good");
  assert("No absence dialogue",
    result.return_dialogue_tier, "no_absence");
})();
...
(() => {
  ...
  assert("Low genki + long absence: 20 → 0 → 20",
    result.genki_value, 20);
...
(() => {
  const result = calculateDecay(80, daysAgo(1, NOW), NOW);
  assert("AC2: 24h inactive → -10 points",
    result.decay_applied, 10);
})();

// AC3: First open of day adds 20
(() => {
  const yesterday = new Date("2026-03-24T18:00:00Z");
  const result = applyDailyCheckin(50, yesterday, NOW);
  assert("AC3: First daily open → +20",
    result.daily_bonus_applied, 20);
})();
...
(() => {
  ...
  for (let i = 0; i < 6; i++) {
    const result = applyMessageBonus(genki, sessionPoints);
    genki = result.new_genki_value;
    sessionPoints = result.session_total;
  }
  assert("AC3: 6 messages → +10 cap (50 → 60)",
    genki, 60);
  assert("AC3: Session points capped at 10",
    sessionPoints, 10);
})();

// AC7: Genki never below 0
(() => {
  const result = calculateDecay(5, daysAgo(3, NOW), NOW);
  assert("AC7: Never below 0", result.new_genki_value >= 0, true);
})();

// AC7: Genki never above 100
(() => {
  const result = applyDailyCheckin(95, daysAgo(1, NOW), NOW);
  assert("AC7: Never above 100",
    result.new_genki_value <= 100, true);
})();
