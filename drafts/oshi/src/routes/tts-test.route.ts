...
/** Request body shape for the TTS test endpoint */
interface TTSTestBody {
  text: string;
}

export async function ttsTestRoutes(
  fastify: FastifyInstance
): Promise<void> {
  fastify.post(
    "/api/tts/test",
    async (
      request: FastifyRequest<{ Body: TTSTestBody }>,
      reply: FastifyReply
    ) => {
      ...
      try {
        ...
        const audioComplete = new Promise<Buffer>(
          (resolve, reject) => {
        ...
        reply.header(
          "X-TTS-Latency-Ms",
          endToEndLatencyMs.toString()
        );
        reply.header(
          "X-TTS-Chunks",
          audioChunks.length.toString()
        );
